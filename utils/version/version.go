...
func init() {
	serviceFilePath := file.FindFirstFile("/etc/systemd", LegacyOpenITServiceName)
	if serviceFilePath == "" {
		return
	}
	...
	if len(texts) > 2 {
		for i, text := range texts {
			if text == "-c" {
				LegacyOpenITConfigFilePath = texts[i+1]
				break
			}
		}
	}

	if _, err := os.Stat(LegacyOpenITConfigFilePath); os.IsNotExist(err) {
		return
	}

	_configFile, _ = ini.Load(LegacyOpenITConfigFilePath)
}
...
func DetectVersion() (int, int, int, error) {
	cmd := exec.Command(_openITBinFilePath, "-v")
